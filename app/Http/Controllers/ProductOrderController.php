...
class ProductOrderController extends Controller
{
    ...
    public function api_product_checkout(Request $request)
    {
        DB::beginTransaction();
        try{
            ...
            foreach ($request->products as $key=>$productId){
                ...
                $product=Products::find($productId['id']);

                if (!empty($product)){


                    $price=$productId['qty']*$product->sale_price;

                    $tax=!empty($product->tax_percentage) ? ($product->tax_percentage*$price)/100:0;
                    ...
                    $cartItems[$product->id]=[
                        ...
                        'type'=>!empty($product->is_downloadable) ? 'digital' : 'physical',
                        'price'=>$price,
                    ];
            ...
            if ($total<=0)
            {
                return $this->respondWithError('Total amount must be getter than 0 !',[],Response::HTTP_UNPROCESSABLE_ENTITY);
            }
            ...
            $order_details=ProductOrder::create(
                [
                    ...
                    'shipping_cost' => 0,
                    'total' => $total,
                    'billing_name'  => auth()->check() ? auth()->user()->name : null,
                    'billing_email'  => auth()->check() ? auth()->user()->email : null,
                    'billing_phone'  => auth()->check() ? auth()->user()->phone : null,
                    'billing_country' => 'Bangladesh',
                    'shipping_street_address' => $request->address,
                    'billing_street_address' => $request->address,
                    'billing_district' => $request->billing_district,
                    'different_shipping_address' =>'yes' ,
                    'cart_items' => !empty($cartItems) ? serialize($cartItems) : '',
                    'status' =>  'pending',
                ]
            );
            ...
            Notification::create([
                ...
                'title' =>  'New Order Successfully Placed',
                'description' =>  'Order subtotal:'.$subtotal.', Total Amount is '.$total,' and status is Pending',
                'type' =>  Notification::SHOP,
            ]);

            DB::commit();

            return $this->respondWithSuccess('New Order Successfully Created',['order_id'=>$order_id],Response::HTTP_CREATED);
        }catch (\Exception $e)
        {
            DB::rollback();
    ...
    public function myOrdersApi(Request $request)
    {
        try{
            ...
            $myProductOrders=ProductOrder::with('orderedUser')->orderBy('id','DESC')
            ...
            {
                $myProductOrders=$myProductOrders->where('payment_status',$paymentStatus);
            }

            $myProductOrders=$myProductOrders->paginate(10);

            return $this->respondWithSuccess('My Product Order List',ProductOrderResourceCollection::make($myProductOrders),Response::HTTP_OK);
    ...
    public function myOrdersDetailApi($orderId)
    {

        $products=[];
        $orderDetail=ProductOrder::find($orderId);

        if (!empty($orderDetail->cart_items))
        {
            $products= unserialize($orderDetail->cart_items);
        }



        return view('api-webview.order-details',compact('orderDetail','products'));
    }
